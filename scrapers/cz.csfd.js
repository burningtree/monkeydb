...
exports.job = new nodeio.Job({timeout: 10}, {
    ...
    run: function(url) {
        this.getHtml(url, function(err, $) {
            ...
            output.name = $('h1').text;
            output.links = [];
            try { output.links.push($('ul.links a.www').attribs.href); } catch(err) {}
            output.image = $('meta[property="og:image"]').attribs.content;
            var domain = $('meta[name="msapplication-starturl"]').attribs.content;
            output.url = domain+$('form#frm-pof-ordererSelectForm').attribs.action.match(/^\/([^\?]+)\?/)[1];

            output.links.push($('ul.links a.imdb').attribs.href);
